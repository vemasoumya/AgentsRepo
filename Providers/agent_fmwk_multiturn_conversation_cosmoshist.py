...
# Add references
from agent_framework import tool, Agent, CompactionProvider
...
from cosmosdb_history_provider import CosmosDBHistoryProvider
...
async def start_conversation():
    ...
    history_provider = CosmosDBHistoryProvider(
        endpoint=cosmos_endpoint,
        database_name="AgentHistory",
        container_name="HotelConversations",
    ...
    before = DebugStrategy(
         "BEFORE_STRATEGY",SelectiveToolCallCompactionStrategy(keep_last_tool_call_groups=1)
    )
    ...
    try:
        async with (
            Agent(
                ...
                context_providers=[history_provider, compaction],  # TEMPORARILY REMOVED history_provider to test
